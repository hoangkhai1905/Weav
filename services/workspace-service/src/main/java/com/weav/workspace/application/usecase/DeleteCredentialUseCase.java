...
@Service
public final class DeleteCredentialUseCase {
    ...
    public ConnectionResponse execute(UUID actorUserId, UUID workspaceId, UUID connectionId) {
        ...
        return usageProtection.reauthorizeAndMutate(
                ...
                (membership, connection) -> deleteInTransaction(membership, connection));
    }

    private ConnectionResponse deleteInTransaction(Membership membership, Connection connection) {
        credentialRepository.deleteByConnectionId(connection.getId());
        if (connection.getAuthType() != ConnectionAuthType.NONE
                && connection.getStatus() != ConnectionStatus.DISABLED) {
            connection.markDisabled();
            connectionRepository.save(connection);
        }
        Credential remaining = credentialRepository.findByConnectionId(connection.getId()).orElse(null);
        return viewAssembler.assemble(connection, membership, remaining);
    }
}
